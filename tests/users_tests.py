...
class TestUsers:
    ...
    def test_post_user(self, app_url, name, email, id):
        ...
        resp_get_user = requests.get(f"{config.base_url}/users/{id}")
    ...
    def test_post_user_invalid_id(self, name, email, id):
        body = {
            "name": name,
            "email": email,
            "id": id
        }
        resp_create_user = requests.post(f"{config.base_url}/users", json=body)
    ...
    def test_post_user_no_id(self, name, email, id):
        body = {
            "name": name,
            "email": email,
        }
    ...
    def test_delete_user(self, user_id):
        ...
        requests.post(f"{config.base_url}/users", json=body)
        resp_delete_user = requests.delete(f"{config.base_url}/users/{user_id}")
        assert resp_delete_user.status_code == 200
        resp_get_user = requests.get(f"{config.base_url}/users/{user_id}")
        assert resp_get_user.status_code == 404
        assert resp_get_user.json()["detail"] == "User not found"

    @pytest.mark.parametrize("user_id", [123])
    def test_delete_user_id_not_exist(self, user_id):
        body = {
            "name": "Petr Van",
            "email": "petya_van@mail.ru",
            "id": user_id
        ...
        requests.post(f"{config.base_url}/users", json=body)
        resp_delete_user = requests.delete(f"{config.base_url}/users/{321}")
        assert resp_delete_user.status_code == 404
        assert resp_delete_user.json()["detail"] == "User not found"


    def test_delete_user_id_already_exist(self):
        user_id: int = 7
        body = {
            "name": "Petr Van",
            "email": "petya_van@mail.ru",
        ...
        while requests.post(f"{config.base_url}/users", json=body).status_code != 200:
            user_id = random.randint(0, 100)
            body["id"] = user_id
        resp_post_user = requests.post(f"{config.base_url}/users", json=body)
